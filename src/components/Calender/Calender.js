...
//       // const lastDayOfPreviousMonth = new Date(currentYear, currentMonth, 0).getDay(); // 0 means last day of previous month
//       // const totalDays = daysInMonth(currentYear, currentMonth); // Total days in the current month
//       // const days = Array.from({ length: totalDays }, (_, i) => i + 1); // Array of days for the current month
    
//       // return [
//       //   // Empty slots for alignment based on the last day of the previous month
//       //   ...Array(lastDayOfPreviousMonth + 1).fill(null), // Adjust for 0-indexed days
//       //   ...days.map((day) => {
...
//   const totalDays = daysInMonth(currentYear, currentMonth); // Total days in the current month
//   const days = Array.from({ length: totalDays }, (_, i) => i + 1); // Days for the current month

//   return [
//     // Empty slots for alignment based on the first day of the current month
...
//           {renderCalendarDays()}
//         </div>
//         <div className='hidden md:block fixed bottom-4 px-2 left-0 right-0 justify-center md:justify-center z-10'>
//           <Logo />
//         </div>
//       </div>
...
const Calendar = () => {
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 1; i <= lastDateOfMonth; i++) {
      ...
      days.push(
        <div
          key={`curr-${i}`}
          className={`lg:h-14 xl:h-24 2xl:h-32 md:h-16 flex items-center xl:text-5xl text-black md:text-3xl justify-center rounded-full cursor-pointer transition-all ${
            isToday
              ? 'bg-white font-bold'
              : occasion
              ? 'border border-cyan-500'
  ...
  return (
    <div className="h-screen flex flex-col md:flex-row font-Montserrat">
      <div className="w-full md:w-1/2 min-h-min p-6 bg-gradient-to-r from-gray-300 via-yellow-500 to-amber-400 md:overflow-y-auto">
        <div className="flex justify-between items-center lg:mb-16 mb-6">
          <button
            className="p-2 rounded-full bg-black text-white"
            onClick={handlePreviousMonth}
